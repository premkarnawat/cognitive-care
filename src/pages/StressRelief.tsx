...
interface TipVideo {
  ...
  title: string;
  description: string;
  video_url?: string;   // frontend still uses video_url
  category: string;
  type: string;
}
      ...
      <div className="relative z-10 px-5 pt-8">
        ...
        >
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl glass-card"
          >
        ...
        {loading ? (
          ...
          <div className="glass-card p-8 text-center">
            ...
            <p className="text-muted-foreground">
              No stress relief content yet. Check back soon!
            </p>
          </div>
          ...
          <Tabs defaultValue="tips" className="w-full">
            ...
            <TabsContent value="tips" className="space-y-3">
              {tips.length > 0
                ? tips.map(t => <ContentCard key={t.id} item={t} />)
                : <p className="text-center text-muted-foreground text-sm py-8">No tips yet.</p>}
            </TabsContent>

            <TabsContent value="videos" className="space-y-3">
              {videos.length > 0
                ? videos.map(v => <ContentCard key={v.id} item={v} />)
                : <p className="text-center text-muted-foreground text-sm py-8">No videos yet.</p>}
            </TabsContent>

            <TabsContent value="audio" className="space-y-3">
              {audio.length > 0
                ? audio.map(a => <ContentCard key={a.id} item={a} />)
                : <p className="text-center text-muted-foreground text-sm py-8">No audio yet.</p>}
            </TabsContent>
